...
```
  ...
  ✗ POST /usuario → 400 — expected 201 + {id}, got 400 {"error":"Contrasenya obligatòria"}
  ✓ GET /domicili → 200
  ...
  ✗ PUT /domicili/999999 (nonexistent) → 500 — expected 404, got 500
  ✓ DELETE /domicili/999999 (nonexistent) → 404
  ✗ POST /domicili → 500 — expected 201 + {id}, got 500 {"message":"Error creant domicili"}
  ✓ GET /familia → 200
  ...
  ✗ PUT /client/999999 (nonexistent) → 500 — expected 404, got 500
  ...
  ✗ POST /client/full → 500 — expected 201 + {id}, got 500 {"message":"Error creant client complet"}
  ✓ POST /client/full missing fields → 400
  ✗ POST /client/full amb familia/domicili existents → 500 — expected 201 + {id}, got 500 {"message":"Error creant client complet"}
```
