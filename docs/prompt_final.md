...
You are building the complete FunEduca CRM from the existing codebase at `"C:\Users\angel\OneDrive\Escritorio\A\Practicas_funeduca_terrassa"`. The backend uses Express 5 + mysql2/promise with raw SQL. The frontend is static HTML/CSS/JS served from `src/public/`. Architecture: `routes/{entity}.js → controllers/{entity}.js → repositories/{entity}.js`.
